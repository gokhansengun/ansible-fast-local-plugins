...
A fast path that implements only a subset of its stock counterpart's arguments must carry a `_SUPPORTED_ARGS` frozenset (canonical names — aliases are folded first) and delegate whenever `set(args) - _SUPPORTED_ARGS` is non-empty, passing the unsupported names to the strict guard via `extra_reasons`. Without the gate an unimplemented argument is *silently ignored* — the task runs fast, reports ok, carries the marker, and did something different from stock (this is how `stat`'s `checksum:` produced sha1 and `helm_info`'s `namespace:` queried the wrong namespace before the alias work). `uri`, `get_url`, `lineinfile`, `stat`, `hashivault_read`, and all five `kubernetes.core` overrides now gate this way; keep each set exactly in sync with what its `_run_local` reads. Values can gate conditionally where only some are reproducible in-process — `uri` delegates `body_format=form-multipart`; `k8s`/`k8s_info` honour `validate_certs=false` (kubectl `--insecure-skip-tls-verify`) but delegate an explicit `true`, since enforcing verification over the kubeconfig is something only the genuine client can do.

### Argspec type coercion (`type=bool` / `type=int` / `type=list`)

Same root cause as the alias gap: stock coerces argument *types* inside `AnsibleModule` (`check_type_bool`/`_int`/`_list`), which the fast path never runs — so a fast plugin sees whatever YAML/templating produced. A templated value is frequently a **string**, and the raw string misbehaves silently:

- **`type=bool`** — `if args.get('flag')` treats the string `"false"`/`"no"`/`"0"` as truthy (non-empty string). Always read bool args through `boolean(x, strict=False)` (standalone plugins) or the self-contained `_bool_arg(x)` (the k8s/helm overrides), never bare truthiness or `bool(x)` (`bool("false")` is `True`). This is why `command`/`shell` (`stdin_add_newline`, `strip_empty_ends`), `k8s` (`force`, `apply`, `wait`), `hashivault_read` (`verify`), and the `helm_*` flag gates (`pass_credentials`, `force_update`, `provenance`, …) coerce.
- **`type=int`** — `int("200,204")` on `uri`'s `status_code` was the real regression that started this; a comma string is a *list*, see below. Plain int args (`uri`/`get_url` `timeout`, `hashivault_read` `timeout`/`secret_version`) are wrapped in `int(...)` so a string `"30"` doesn't reach `open_url`/`hvac.Client` as a non-numeric timeout.
- **`type=list`** (incl. `elements=int`) — stock's `check_type_list` **comma-splits a bare string** (`"200,204"` → `["200","204"]`, `"app=foo,tier=bar"` → two selectors) and wraps a scalar. Without this, `[int(s)]` throws or `','.join(s)` explodes the string character-by-character. `uri` (`status_code`) inlines the split; `k8s`/`k8s_info` (`label_selectors`/`field_selectors`) and `helm_info` (`release_state`) use a shared-shape `_as_list()` helper.

When adding a plugin, check the stock module's argspec for `type=bool/int/list` on any argument `_run_local` reads and coerce it — a raw read is a silent-parity bug the `_SUPPORTED_ARGS` gate won't catch (the arg *is* supported; only its type is wrong).

### Error-message parity (`hashivault_read`)

A stock module's *failure text* can be a behavioural contract, not just human-facing prose. `hashivault_read` is the case that proved it: stock catches `hvac.exceptions.InvalidPath` and reports a missing secret as `Secret <mount>/<secret> is not in vault`, which callers match on to implement read-or-create ("read the cert from Vault; generate and store it if absent"). The fast path originally funnelled *every* exception into `vault read failed: %s`, so a missing secret surfaced as hvac's opaque `None, on get <url>` — the guard never matched and first-run provisioning failed hard across five product namespaces. `_run_local` now mirrors the stock function's control flow exactly: `InvalidPath` → not-found branch, the `default:` argument short-circuiting both the missing-secret and missing-key cases to a successful result, stock's `Error <Class>(<msg>) reading <path>` wording for genuine errors, and stock's leading-slash rule (`secret: /a/b` drops the mount point) since `secret_path` appears in those messages.

Because the shared `test_parity` harness denylists `msg` and requires a marked (i.e. successful) fast result, this contract has its own harness: `hashivault_not_found_parity.yml` + `test_hashivault_read_not_found_parity`, which runs the same missing read fast and under `AFLP_DISABLE=1` and compares `failed`/`rc`/`msg`, using a second *successful* read as the path-validity probe. When a fast plugin's callers can branch on an error, compare the error.
...
`copy.py` fast-paths **three** shapes to a local `dest`: inline `content`; a single local-file `src` (resolved via `self._find_needle('files', ...)`, read on the controller, written with `atomic_write`; a directory dest receives the file by basename, matching stock); and a **local-directory `src`**, copied recursively in-process (`_copy_dir`). The directory path honors stock's trailing-slash semantics (`src: dir/` copies the *contents* into dest, `src: dir` nests the dir by basename), creates empty dirs, is per-file checksum-idempotent, and applies `mode` to files / `directory_mode` to dirs (both default to the umask, like stock); its result is `{changed, dest (with trailing slash), src}` — no top-level `checksum`, matching stock's directory result. It falls back for `remote_src`, a **symlink anywhere in the source tree** (the fast path can't reproduce `local_follow`), `mode: preserve`, `directory_mode: preserve`, `both/neither content+src`, and the usual ownership/backup/validate args. Because these copy-specific triggers are invisible to `strict_guard`'s generic detection (non-local / become / check_mode / async), `copy.py` passes them as `extra_reasons=[...]` so `AFLP_STRICT` names the real cause (e.g. `remote_src`, `symlink in source tree`) instead of the catch-all "unsupported arguments". A new override that falls back for plugin-specific reasons should do the same. The directory copy is held to stock byte-for-byte by the `copy_dir` / `copy_dir_slash` parity cases (which compare the whole produced tree's type+mode+relpath and per-file checksums).

### Fast-path marker

Every plugin stamps a successful in-process result with `__produced_by_fast_plugin: True` so tests can prove the fast path ran rather than the standard fallback (whose result never carries it). The standalone plugins import `mark_fast_result` / `FAST_PLUGIN_MARKER` from `_action_utils.py`; the five `kubernetes.core` overrides define a local `mark_fast_result` (they live in the separate `aflp.kubernetes_core` collection and cannot cleanly import `_action_utils` without breaking the file-path unit-test loader, so they stay self-contained). The marker is applied centrally: each `run()` resolves the fallback/delegate gate, then returns `mark_fast_result(self._run_local(...))`. By default only non-failed dicts are marked (`setdefault`, so an explicit value is never clobbered), so a *plugin* failure (it couldn't do its job) and delegated/fallback results stay unmarked. **Exception — `command`/`shell`:** a non-zero command `rc` is the command's outcome, not a fallback, so the result of a command that actually ran is marked via `mark_fast_result(result, force=True)` even when `failed` is True. Otherwise a `failed_when:`-rescued probe (e.g. `kubectl get cm` that exits non-zero when absent) would become an unmarked *ok* result and the summary callback would misreport it as a `fallback`. Their pre-execution bails (no command, missing `chdir`) are returned without `force` and stay unmarked. ansible-core only strips `_ansible_`-prefixed keys, so the marker survives into the registered result; read it in playbooks via subscript (`r['__produced_by_fast_plugin']`), not dotted access, which the templating sandbox blocks for `_`-prefixed names.
...
> Why a callback and not a strategy plugin: custom strategy plugins (the other interception point) are deprecated in ansible-core 2.19 and slated for removal (~2.21) — see ansible/ansible#84725. The patched symbol (`action_loader.get`) is verified against the 2.20 / 2.21 matrix; re-check on a matrix bump.
...
### Semantic parity: the CLI verb is not the module's verb (`k8s`)

`_SUPPORTED_ARGS` catches arguments the fast path ignores, but it cannot catch a *supported* argument mapped onto the wrong `kubectl` verb. `k8s` was the case that proved it: `state: present` was implemented as `kubectl apply`, while `kubernetes.core` defaults `apply` to **false** and does create-or-patch. The two differ exactly where it hurts — `kubectl apply` three-way-merges against the `last-applied-configuration` annotation and **deletes every field the manifest omits**, so a task passing a *partial* definition (the `hostAliases`-only patch a role applies to Rancher's `cattle-cluster-agent`, itself created by `kubectl apply`) nulled `spec.selector` and `spec.template.spec.containers` and was rejected by the API server. A plain PATCH never prunes.

`_run_local` now mirrors `perform_action()` in `kubernetes.core`'s `module_utils/k8s/runner.py` object by object: `apply: true` → `kubectl apply`; absent → `kubectl create` (`state: patched` warns instead and creates nothing); `force: true` → `kubectl replace` (a PUT, *not* `kubectl apply --force`, which deletes and recreates); otherwise `kubectl patch`, trying each `merge_type` in turn (`strategic-merge` then `merge` by default, so a CRD that rejects strategic falls through). `changed` compares the pre-fetched object with the returned one and ignores `metadata.generation`/`resourceVersion`, mirroring `diff_objects()`; the result shape (`method`, and `result.results` for a multi-document definition) mirrors `run_module()`. When adding or changing an override, read the genuine module's dispatcher, not just its argspec.
