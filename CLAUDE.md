...
- **`type=bool`** — `if args.get('flag')` treats the string `"false"`/`"no"`/`"0"` as truthy (non-empty string). Always read bool args through `boolean(x, strict=False)` (standalone plugins) or the self-contained `_bool_arg(x)` (the k8s/helm overrides), never bare truthiness or `bool(x)` (`bool("false")` is `True`). This is why `command`/`shell` (`stdin_add_newline`, `strip_empty_ends`), `k8s` (`force`, `apply`, `wait`), and the `helm_*` flag gates (`pass_credentials`, `force_update`, `provenance`, …) coerce.
- **`type=int`** — `int("200,204")` on `uri`'s `status_code` was the real regression that started this; a comma string is a *list*, see below. Plain int args (`uri`/`get_url` `timeout`) are wrapped in `int(...)` so a string `"30"` doesn't reach `open_url` as a non-numeric timeout.
