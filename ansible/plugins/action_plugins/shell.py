...
class ActionModule(ActionBase):
    ...
    def _run_local(self, args, result):
        ...
        # chdir is commonly supplied via `args: {chdir: ...}`; ansible declares it
        # as a path arg, so expand ~ and env vars to match stock (creates/removes
        # below do the same). Fail clearly on a missing dir instead of surfacing a
